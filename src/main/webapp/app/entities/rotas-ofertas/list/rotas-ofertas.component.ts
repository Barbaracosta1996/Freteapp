...
import { EntityArrayResponseType, RotasOfertasService } from '../service/rotas-ofertas.service';
...
@Component({
  selector: 'jhi-rotas-ofertas',
  templateUrl: './rotas-ofertas.component.html',
})
export class RotasOfertasComponent implements OnInit {
  ...
  trackId = (_index: number, item: IRotasOfertas): number => this.rotasOfertasService.getRotasOfertasIdentifier(item);
  ...
  delete(rotasOfertas: IRotasOfertas): void {
    const modalRef = this.modalService.open(RotasOfertasDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.rotasOfertas = rotasOfertas;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.rotasOfertas = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IRotasOfertas[] | null): IRotasOfertas[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.rotasOfertasService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
