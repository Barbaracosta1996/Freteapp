...
import { EntityArrayResponseType, FrotaService } from '../service/frota.service';
...
export class FrotaComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFrota): number => this.frotaService.getFrotaIdentifier(item);

  ngOnInit(): void {
    this.load();
  }

  delete(frota: IFrota): void {
    const modalRef = this.modalService.open(FrotaDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.frota = frota;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.frotas = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IFrota[] | null): IFrota[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.frotaService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
